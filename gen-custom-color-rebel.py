...
def main():
    ...
    color_palette_num = int(script_args[1])
    ...
    for token_id, blender_values in collection_data.items():
        if token_id != rebel_id:
            continue

        print(f"")
        print(f"**** Rendering rebel {rebel_id}")
        print(f"")
        ...
        color_palette_value = color_palette_num

        # If the Rebel has already been rendered, skip
        if path.exists(f"./{OUTPUT_DIRECTORY}/rebel-{rebel_id}.png") and path.getsize(f"./{OUTPUT_DIRECTORY}/rebel-{rebel_id}.png") > 3000000:
            continue
