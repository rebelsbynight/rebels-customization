...
OUTPUT_DIRECTORY = "rendered-rebels"

def main():
    collection_data = {}
    script_args = sys.argv[sys.argv.index("--") + 1:]
    collection_mapping_path = script_args[0]
    custom_balaclava_image = script_args[1]
    rebel_id = script_args[2]
    ...
    for token_id, blender_values in collection_data.items():
        if rebel_id and rebel_id == token_id:
            print(f"")
            ...
            # Only use the NFT's current balaclava if no customized balaclava is provided
            if custom_balaclava_image != None and len(custom_balaclava_image) != 0:
                # If the custom balaclava image is provided, use it
                material = bpy.data.materials["gvdsyuqgu.008"]
                material.node_tree.nodes["Image Texture.009"].image = bpy.data.images.load(custom_balaclava_image)
            else:
                # Otherwise use the NFT's current balaclava
